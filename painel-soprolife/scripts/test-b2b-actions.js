...
const clin = (extra) => ({ clinica_id: "CLI-0001", nome_clinica: "Clínica Exemplo",
  etapa: "Em conversa", tem_proxima_acao: true, ...extra });
...
const a1 = buildB2BActions({ clinicas: [clin(), clin({ clinica_id: "CLI-0002",
  nome_clinica: "Clínica Norte", etapa: "Aguardando retorno" })] }, AGORA);
caso("em conversa -> ação média 'manter conversa'",
     a1.some((a) => a.prioridade === "media" && /Manter conversa/.test(a.titulo)));
caso("aguardando retorno -> ação média 'reforçar contato'",
     a1.some((a) => a.prioridade === "media" && /Reforçar contato/.test(a.titulo)));
const s1 = buildB2BStats({ clinicas: [clin(), clin({ etapa: "Aguardando retorno" })] }, AGORA);
...
const s2 = buildB2BStats({ clinicas: [clin({ etapa: "Parceiro ativo" })],
...
const a2 = buildB2BActions({ leads: [leadB2B({ etapa: "Parceiro ativo" })] }, AGORA);
...
caso("desistiu não gera ação",
     buildB2BActions({ leads: [leadB2B({ etapa: "Desistiu" })] }, AGORA).length === 0);

// 6. Sem próximo passo -> ação média + stat
const a3 = buildB2BActions({ clinicas: [clin({ etapa: "Abordada", tem_proxima_acao: false })] }, AGORA);
...
caso("stat semProximoPasso conta",
     buildB2BStats({ clinicas: [clin({ etapa: "Abordada", tem_proxima_acao: false })] }, AGORA).semProximoPasso === 1);

// 7. Prioridade ALTA por atraso (data_proxima_acao vencida)
const a4 = buildB2BActions({ leads: [leadB2B({ etapa: "Aguardando retorno",
...
caso("proposta enviada -> alta 'cobrar retorno'",
     a5.some((a) => a.prioridade === "alta" && /Cobrar retorno da proposta/.test(a.titulo)));
...
const suspeitos = buildB2BActions({ clinicas: [
  clin({ etapa: "Proposta enviada", nome_clinica: "ligar (21) 99999-8888" }),
  clin({ clinica_id: "CLI-9", etapa: "Em conversa", nome_clinica: "contato x@y.com" }),
  clin({ clinica_id: "CLI-10", etapa: "Pediu apresentação", nome_clinica: "doc 123.456.789-09" }),
  clin({ clinica_id: "CLI-11", etapa: "Em conversa", nome_clinica: "chave ya29.abc" }),
] }, AGORA);
...
const comExtra = buildB2BActions({ clinicas: [clin({ etapa: "Proposta enviada",
...
const muitas = buildB2BActions({ clinicas: Array.from({ length: 20 }, (_, i) =>
  clin({ clinica_id: `CLI-${i}`, nome_clinica: `Clínica ${i}`,
         etapa: i % 2 ? "Proposta enviada" : "Em conversa" })) }, AGORA);
...
const a6 = buildB2BActions({ clinicas: [clin({ etapa: "Parceira" })] }, AGORA);
caso("etapa fora do padrão -> ação 'revisar etapa'",
     a6.some((a) => a.prioridade === "media" && /Revisar etapa no CRM/.test(a.titulo)));
caso("etapa fora do padrão citada no motivo", a6.some((a) => /Parceira/.test(a.motivo)));
caso("etapa fora do padrão não conta como oportunidade",
     buildB2BStats({ clinicas: [clin({ etapa: "Parceira" })] }, AGORA).totalOportunidades === 0);
